	...
	extends SimpleValueCriteria
{
	private static final String DESCRIPTION = """
		Criteria used to match a LocalDateTime.

		All fields within this criteria are nullable, but exactly one field
		needs to be present for the criteria to be valid.

		* `any` can be set to `true` to match if any LocalDateTime is present, or
		  `false` to only match if no LocalDateTime is present.
		* `equals` can be used to match a value exactly.
		* `range` can be used to match if a LocalDateTime is within the given range.
	""";

	private static final String RANGE_DESCRIPTION = """
		Criteria used to match if a LocalDateTime is within a certain range. This
	...
	{
		this.graphQLType = InputObjectDef.create("LocalDateTimeCriteriaInput")
			...
			.addField(InputFieldDef.create("equals")
				...
				.withDescription("LocalDateTime should equal the given value")
				.build()
			)
			.addField(InputFieldDef.create("range")
				.withType(createRange())
				.withDescription("LocalDateTime should be within the given range")
				.build()
			)
			.build();
	}
	...
	{
		return InputObjectDef.create("LocalDateTimeRangeCriteriaInput")
