	...
	extends SimpleValueCriteria
{
	private static final String DESCRIPTION = """
		Criteria used to match a LocalDate.

		All fields within this criteria are nullable, but exactly one field
		needs to be present for the criteria to be valid.

		* `any` can be set to `true` to match if any LocalDate is present, or
		  `false` to only match if no LocalDate is present.
		* `equals` can be used to match a value exactly.
		* `range` can be used to match if a LocalDate is within the given range.
	""";

	private static final String RANGE_DESCRIPTION = """
		Criteria used to match if a LocalDate is within a certain range. This
	...
	{
		this.graphQLType = InputObjectDef.create("LocalDateCriteriaInput")
			...
			.addField(InputFieldDef.create("equals")
				...
				.withDescription("LocalDate should equal the given value")
				.build()
			)
			.addField(InputFieldDef.create("range")
				.withType(createRange())
				.withDescription("LocalDate should be within the given range")
				.build()
			)
			.build();
	}
	...
	{
		return InputObjectDef.create("LocalDateRangeCriteriaInput")
