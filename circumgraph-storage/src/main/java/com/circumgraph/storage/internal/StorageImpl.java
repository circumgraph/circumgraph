	...
	implements Storage
{
			...
			for(var subDef : unionDef.getTypes())
			{
				...
				if(subDef.findImplements(StorageSchema.ENTITY_NAME))
				{
					...
					collectSimpleValueIndexer(
						...
						generator,
						fieldReceiver,
						AutoGeneratedIdValueIndexer.INSTANCE,
						false,
						false
					);
				}
	...
	private static Object extractValue(Value v)
	{
		...
		if(v instanceof SimpleValue simpleValue)
		{
			return simpleValue.get();
		}
		else if(v instanceof StoredObjectRef storedObjectRef)
		{
			return storedObjectRef.getDecodedId();
		}
