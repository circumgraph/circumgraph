...
import com.circumgraph.storage.internal.serializers.AutoGeneratedIdValueSerializer;
...
import com.circumgraph.storage.internal.serializers.ScalarValueSerializer;
...
public class ValueSerializers
{
	...
	)
	{
		...
		scalars = Scalars.instance()
			.list()
			.toMap(Scalar::getModelType, scalar -> new ScalarValueSerializer(scalar));
	}
		...
		return new StructuredValueSerializer(
			...
			def.getFields()
				...
				.toMap(FieldDef::getName, f -> {
					if(StorageSchema.ENTITY_NAME.equals(f.getDeclaringType().getName())
						&& "id".equals(f.getName()))
					{
						// Entity id
						return AutoGeneratedIdValueSerializer.INSTANCE;
					}
