	...
	extends SingleSchemaTest
{
	...
	public void testStore()
	{
		...
		assertThat(createdValue.get(), is(LocalDateTime.of(2020, 1, 1, 8, 45, 30, 100000000)));
	}
	...
	public void testQueryExactMatch()
	{
		...
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(EqualsMatcher.create(LocalDateTime.of(2020, 1, 1, 8, 45, 30, 100000000))))
		).block();
	...
	public void testQueryExactNoMatch()
	{
		...
		var root = QueryPath.root(collection.getDefinition());
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(EqualsMatcher.create(LocalDateTime.of(2020, 1, 1, 8, 45, 30, 200000000))))
		).block();
	...
	public void testQueryRangeIncluded()
	{
		...
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(RangeMatcher.isMoreThan(
				LocalDateTime.of(2020, 1, 1, 8, 45, 30)
			)))
		).block();
