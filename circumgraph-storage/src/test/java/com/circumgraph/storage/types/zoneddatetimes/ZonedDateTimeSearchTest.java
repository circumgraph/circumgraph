...
public class ZonedDateTimeSearchTest
	extends SingleSchemaTest
{
	...
	public void testStore()
	{
		...
		var mutation = collection.newMutation()
			.updateField("created", ScalarValueMutation.create(
				...
				ZonedDateTime.of(2020, 1, 1, 8, 45, 30, 100000000, ZoneOffset.ofHours(2))
			))
			.build();
		...
		assertThat(createdValue.get(), is(ZonedDateTime.of(2020, 1, 1, 8, 45, 30, 100000000, ZoneOffset.ofHours(2))));
	}
	...
	public void testQueryExactMatchDifferentZone()
	{
		...
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(EqualsMatcher.create(
				ZonedDateTime.of(2020, 1, 1, 6, 45, 30, 100000000, ZoneOffset.UTC)
			)))
		).block();
	...
	public void testQueryExactNoMatch()
	{
		...
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(EqualsMatcher.create(
				ZonedDateTime.of(2020, 1, 1, 8, 45, 30, 200000000, ZoneOffset.UTC)
			)))
		).block();
	...
	public void testQueryRangeIncluded()
	{
		...
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(RangeMatcher.isMoreThan(
				ZonedDateTime.of(2020, 1, 1, 8, 45, 30, 0, ZoneOffset.ofHours(2))
			)))
		).block();
