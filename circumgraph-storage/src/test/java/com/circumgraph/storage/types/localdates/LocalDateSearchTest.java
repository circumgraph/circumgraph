	...
	extends SingleSchemaTest
{
	...
	public void testQueryExactMatch()
	{
		...
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(EqualsMatcher.create(LocalDate.of(2020, 1, 1))))
		).block();
	...
	public void testQueryExactNoMatch()
	{
		...
		var root = QueryPath.root(collection.getDefinition());
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(EqualsMatcher.create(LocalDate.of(2021, 1, 1))))
		).block();
	...
	public void testQueryRangeIncluded()
	{
		...
		var root = QueryPath.root(collection.getDefinition());
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(RangeMatcher.isMoreThan(LocalDate.of(2019, 12, 31))))
		).block();
	...
	public void testQueryRangeExcluded()
	{
		...
		var root = QueryPath.root(collection.getDefinition());
		var results = collection.search(Query.create()
			.addClause(root.field("created").toQuery(RangeMatcher.isMoreThan(LocalDate.of(2020, 1, 2))))
		).block();
